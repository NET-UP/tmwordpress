				...
				$tm_output .= '<div class="col-12">';
					...
					foreach($events as $event){
						...
						if($atts['show_image'] > 0){
							$tm_output .= '<a class="mr-3 media-img" href="/' . $globals->event_slug . '?id=' . tm_id_encrypt($event->id) . '" style="background-image:url('. $event->event_img_url .')"></a>';
						}
											
							$tm_output .= '<div class="media-body">';
							$tm_output .= '<h5 class="mt-0 mb-1"><a href="/' . $globals->event_slug . '?id=' . tm_id_encrypt($event->id) . '">' . $event->ev_name . '</a></h5>';
			...
			if(empty($events)) {	
				...
				foreach($events as $event) {
					...
					if ($i == 0 || date_i18n( $globals->group_by , strtotime( $curr ) ) != date_i18n( $globals->group_by, strtotime( $prev ) ) ) {
						$tm_output .= "<div class='col-12 mt-2'><h5 class='line-header'><span>" . date_i18n( $globals->format_date, strtotime(iso8601_to_datetime($event->ev_date)) ) . "</span></h5></div>";
						$prev = $curr;
					}
