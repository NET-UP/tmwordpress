            ...
            if(empty($events)) {	
                ...
                $ticketmachine_output .= '<div class="col-12">';
                    ...
                    if($meta['has_previous_page']) {
                        ...
                        $ticketmachine_output .= "<a class='btn btn-secondary mr-3' href='" . strtok($_SERVER["REQUEST_URI"], '?') . "?" . $query_result . "'>" . __("Previous", "ticketmachine-event-manager") . " <i class='fas fa-angle-right ml-2'></i></a>";
                    }

                    if($meta['next'] < $meta['count_filtered']) {
                        $query['pg'] = $params['pg']+1;
                        $query_result = http_build_query($query);
                        $ticketmachine_output .= "<a class='btn btn-secondary' href='" . strtok($_SERVER["REQUEST_URI"], '?') . "?" . $query_result . "'>" . __("Next", "ticketmachine-event-manager") . " <i class='fas fa-angle-right ml-2'></i></a>";
                    }
                    $ticketmachine_output .= '</div>';
