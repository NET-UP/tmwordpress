	...
	function ticketmachine_event_list_item ( $event, $globals ) {
		...
		$ticketmachine_output = '<div class="col-12 col-md-6 col-xl-4 card-group">';
			$ticketmachine_output .= '<card class="card mb-4">';
				$ticketmachine_output .= '<a aria-label="' . esc_attr($event->ev_name) . ' am ' . ticketmachine_i18n_date("d. F Y", $event->ev_date) . '" href="/' . esc_html($globals->event_slug) .'?id=' . esc_html($event->id) . '" class="card-img-top" style="background-image:url( ' . esc_url($event->event_img_url) . ' )" title="' . esc_attr($event->ev_name) . '">';
				  ...
				  $ticketmachine_output .= '<div class="row pt-2">';
					...
					if(!empty($event->ev_location_name)){
						$ticketmachine_output .= '<p class="card-text mt-0 px-2 pt-sm-1 pb-3 pb-sm-2 ellipsis">';
							...
							$ticketmachine_output .= '<a aria-label="' . esc_attr__("Event Location", 'ticketmachine-event-manager') . ': ' . esc_html($event->ev_location_name) . '" href="' . esc_url($globals->map_query_url . urlencode($event->ev_location_name . " " . $event->event_location->street . " " . $event->event_location->house_number . " " . $event->event_location->zip . " " . $event->event_location->city . " " . $event->event_location->country )) . '" target="_blank" title="' . esc_html__("Event Location", 'ticketmachine-event-manager') . ': ' . $event->ev_location_name . '">' . $event->ev_location_name . '</a>';
						$ticketmachine_output .= '</p>';
					}
					$ticketmachine_output .= '</div>';
					$ticketmachine_output .= '<div class="col-sm-4 col-md-5">';
					  $ticketmachine_output .= '<a aria-label="' . esc_attr__("To ticket selection for", 'ticketmachine-event-manager') . ' ' . esc_html($event->ev_name)  . '" href="/' . esc_html($globals->event_slug) .'/?id=' . esc_html($event->id) . '" class="btn btn-primary btn-sm px-3 float-sm-right d-block" title="' . esc_html__("To ticket selection", 'ticketmachine-event-manager') . '">';
