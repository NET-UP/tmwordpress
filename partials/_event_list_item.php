	...
	function tm_event_list_item ( $event, $globals ) {
		...
		$tm_output = '<div class="col-12 col-md-6 col-xl-4 card-group">';
			$tm_output .= '<card class="card mb-4">';
				$tm_output .= '<a aria-label="' . $event->ev_name . ' am ' . tm_i18n_date("d. F Y", $event->ev_date) . '" href="/' . $globals->event_slug .'?id=' . $event->id . '" class="card-img-top" style="background-image:url( ' . $event->event_img_url . ' )" title="' . $event->ev_name . '">';
				...
				$tm_output .= '<div class="card-body position-relative">';
				  ...
				  $tm_output .= '<div class="card-date" title="' . tm_i18n_date("d. F Y", $event->ev_date) . '">';
					$tm_output .= '<div class="card-day">' . tm_i18n_date("d", $event->ev_date) . '</div>';
					$tm_output .= '<div class="card-month">' . tm_i18n_date("M", $event->ev_date) . '</div>';
				  $tm_output .= '</div>';
