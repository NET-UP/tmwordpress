	...
	function tm_event_list_item ( $event, $globals ) {
		...
		$tm_output = '<div class="col-12 col-md-6 col-xl-4 card-group">';
			$tm_output .= '<card class="card mb-4">';
				$tm_output .= '<a aria-label="' . $event->ev_name . ' am ' . date_i18n( "d. F Y", strtotime($event->ev_date), true ) . '" href="/' . $globals->event_slug .'?id=' . $event->id . '" class="card-img-top" style="background-image:url( ' . $event->event_img_url . ' )" title="' . $event->ev_name . '">';
				...
				$tm_output .= '<div class="card-body position-relative">';
				  ...
				  $tm_output .= '<div class="card-date" title="' . date_i18n( "d. F Y", strtotime($event->ev_date), true ) . '">';
					$tm_output .= '<div class="card-day">' . date_i18n( "d", strtotime($event->ev_date), true ) . '</div>';
					$tm_output .= '<div class="card-month">' . date_i18n( "M", strtotime($event->ev_date), true ) . '</div>';
				  $tm_output .= '</div>';
