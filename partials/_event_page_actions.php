        ...
        $ticketmachine_output = '
                <div class="title-height ticketmachine_actions text-right no-height-mobile mb-3 mb-lg-0">
                    ...
                    if ($tm_globals->show_social_media_facebook && $tm_globals->show_social_media) {
                        $ticketmachine_output .= '<a class="btn btn-secondary share-popup mb-1 mb-lg-0 ml-1" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u='. esc_url($url) .'" title="' . esc_attr__('Share to Facebook', 'ticketmachine-event-manager') . '">
                            <i class="fab fa-facebook-f"></i>
                        </a>';
                    }
                    if ($tm_globals->show_social_media_twitter && $tm_globals->show_social_media) {
                        $ticketmachine_output .= '<a class="btn btn-secondary share-popup mb-1 mb-lg-0 ml-1" target="_blank" href="https://twitter.com/intent/tweet?text='. esc_url($url) .'" title="' . esc_attr__('Share to Twitter', 'ticketmachine-event-manager') . '">
                            <i class="fab fa-twitter"></i>
                        </a>';
                    }
                    if ($tm_globals->show_social_media_email && $tm_globals->show_social_media) {
                        $ticketmachine_output .= '<a class="btn btn-secondary mb-1 mb-lg-0 ml-1" href="mailto:?subject='. $event->ev_name .'&body='. esc_url($url) .'" title="' . esc_attr__('Share via Email', 'ticketmachine-event-manager') . '">
                            <i class="fas fa-envelope"></i>
                        </a>';
                    }
                    if ($tm_globals->show_social_media_messenger && $tm_globals->show_social_media) {
                        $ticketmachine_output .= '<a class="btn btn-secondary d-inline-block d-md-none mb-1 mb-lg-0 ml-1" href="fb-messenger://share/?link='. esc_url($url) .'" title="' . esc_attr__('Share with Messenger', 'ticketmachine-event-manager') . '">
                            <i class="fab fa-facebook-messenger"></i>
                        </a>';
                    }
                    if ($tm_globals->show_social_media_whatsapp && $tm_globals->show_social_media) {
                        $ticketmachine_output .= '<a class="btn btn-secondary d-inline-block d-md-none mb-1 mb-lg-0 ml-1" href="WhatsApp://send?text='. esc_url($url) .'" title="' . esc_attr__('Share with WhatsApp', 'ticketmachine-event-manager') . '">
