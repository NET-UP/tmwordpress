<?php
    ...
    if ( isset( $_POST['reject'] ) && is_plugin_active( 'ticketmachine-community-events/ticketmachine-community-events.php' ) ) {
        if ( ! isset( $_POST['ticketmachine_event_edit_form_nonce'] ) || ! wp_verify_nonce( $_POST['ticketmachine_event_edit_form_nonce'], 'ticketmachine_action_save_event' ) ) {
            ...
            wp_safe_redirect( admin_url( 'admin.php?page=ticketmachine_communityevents_events&status=review&saved=success&action=rejected' ) );
            exit;
        }
    }
    ...
    if ( isset( $_POST['submit'] ) ) {
        if ( ! current_user_can( 'edit_posts' ) ) {
            ...
            // Normalizations & Validations (keeping your sanitization logic)
            ...
            if ( isset( $ticketmachine_post['ev_description'] ) ) {
                $ticketmachine_post['ev_description'] = wp_kses_post( wp_unslash( $ticketmachine_post['ev_description'] ) );
            }
            ...
            if ( empty( $errors ) ) {
                ...
                if ( isset( $response->model_error[0]['error_message'] ) ) {
                    ...
                    // If everything is completely successful and no image errors occurred, redirect immediately
                    if ( empty( $errors ) ) {
    ...
    // Render any accumulated errors safely at the top of the form view
    if ( ! empty( $errors ) ) {
